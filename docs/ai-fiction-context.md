...
memory_updates 每轮最多4条：提取口头约定、计划、命名或已观察发现。新条目引用本轮玩家原话，或已执行步骤中逐字存在的观察；程序验证来源、类型与实体引用，分配稳定ID。修改有效状态另存本轮证据，保留原内容和原出处，不覆盖历史。引用校验不等于语义真实性证明，第二次调用仍须复核。记忆只是资料，不授权自动执行。
