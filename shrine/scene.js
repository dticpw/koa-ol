...
// Coordinates: +Z is the entrance; the entire diorama lives on an 18 × 18 plinth.
...
try {
  ...
  notice.textContent = '三维场景需要 WebGL，请开启浏览器硬件加速后重新打开。';
...
if (embedded) {
  ...
  canvas.setAttribute('aria-label','博丽神社微缩模型。左右拖动或使用方向键旋转。完整场景提供缩放功能。');
  window.addEventListener('message',event => {
    if(event.origin !== location.origin || event.source !== window.parent) return;
    if(event.data?.type === 'shrine-visibility') inViewport = event.data.visible === true;
  });
}
canvas.addEventListener('webglcontextlost',()=>notifyParent('shrine-unavailable'));
canvas.addEventListener('webglcontextrestored',()=>{renderer.render(scene,camera);notifyParent('shrine-ready');});
controls.update();
controls.saveState();
...
function bakeStatic(root){
 ...
 for(const {material,geos} of batches.values()){
  ...
  const m=new THREE.Mesh(merged,material);m.castShadow=true;m.receiveShadow=true;scene.add(m);
...
const petalCount=26;
const petals=new THREE.InstancedMesh(new THREE.SphereGeometry(.052,5,3),M.pinkWhite,petalCount);
petals.instanceMatrix.setUsage(THREE.DynamicDrawUsage);scene.add(petals);
const petalData=Array.from({length:petalCount},()=>({x:rand(-7,7),y:rand(2,8),z:rand(-6,5),phase:rand(0,6.28),speed:rand(.18,.38)}));
const dummy=new THREE.Object3D();
const motion=matchMedia('(prefers-reduced-motion: reduce)');
function updatePetals(t){
 petalData.forEach((p,i)=>{
  const y=1.1+((p.y-1.1-t*p.speed)%6.9+6.9)%6.9;
  dummy.position.set(p.x+Math.sin(t*.33+p.phase)*.5,y,p.z+Math.cos(t*.24+p.phase)*.40);
  dummy.rotation.set(t*.7+p.phase,t*.4,p.phase);dummy.scale.set(1.25,.22,.7);dummy.updateMatrix();petals.setMatrixAt(i,dummy.matrix);
...
canvas.addEventListener('keydown',e=>{
 if(e.key==='Home'){
  controls.enableDamping=false;controls.update();controls.reset();controls.enableDamping=true;e.preventDefault();return;
 }
 if(!embedded && ['+','=','-','_'].includes(e.key)){
  camera.zoom=THREE.MathUtils.clamp(camera.zoom*(e.key==='-'||e.key==='_'?.88:1.12),controls.minZoom,controls.maxZoom);camera.updateProjectionMatrix();e.preventDefault();
 }
 if(e.key.startsWith('Arrow')){
  const spherical=new THREE.Spherical().setFromVector3(camera.position.clone().sub(controls.target));
  if(e.key==='ArrowLeft')spherical.theta-=.11;if(e.key==='ArrowRight')spherical.theta+=.11;
  if(e.key==='ArrowUp')spherical.phi-=.08;if(e.key==='ArrowDown')spherical.phi+=.08;
  spherical.phi=THREE.MathUtils.clamp(spherical.phi,controls.minPolarAngle,controls.maxPolarAngle);
  camera.position.copy(new THREE.Vector3().setFromSpherical(spherical).add(controls.target));controls.update();e.preventDefault();
 }
});
