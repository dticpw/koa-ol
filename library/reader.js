...
async function renderHTML(entry,name,source,host,interactive=false){
 ...
 host.replaceChildren();const controls=create('div','reader-actions');const toggle=create('button','',interactive?'切回静态阅读':'启用隔离交互');toggle.onclick=()=>busy(toggle,()=>renderHTML(entry,name,source,host,!interactive));controls.append(toggle);host.append(controls,create('p','reader-meta',interactive?'隔离交互已启用：无法读取馆主页面；fetch、表单与外部子资源受限。需要联网的作品请单独托管。':'静态阅读：脚本不执行；可切换隔离交互查看报告自身的图表与按钮。'));
 if(references.length){const d=create('details','more-resources');d.append(create('summary','',`文内链接 · ${references.length}`));for(const r of references)d.append(link(r.title+' ↗',r.url));host.append(d);}
