...
async function renderHTML(entry,name,source,host,interactive=false,frontpage=false){
 ...
 const mime=n=>({css:'text/css',js:'text/javascript',png:'image/png',jpg:'image/jpeg',jpeg:'image/jpeg',webp:'image/webp',gif:'image/gif',svg:'image/svg+xml',woff2:'font/woff2',woff:'font/woff',ttf:'font/ttf',otf:'font/otf'})[n.split('.').pop().toLowerCase()]||'application/octet-stream';
 async function local(value,base=name){if(/^data:image\/(png|jpeg|webp|gif);base64,/i.test(value))return value;const path=resolve(value,base);const f=entry.files.find(f=>f.name===path);if(!f){missing.add(value);return '';}if(!cache.has(path))cache.set(path,asset(f).then(raw=>{let binary='';for(let i=0;i<raw.length;i+=32768)binary+=String.fromCharCode(...raw.subarray(i,i+32768));return 'data:'+mime(path)+';base64,'+btoa(binary);}));return cache.get(path);}
