...
export function applyMemory(state,next,proposal,actions){
 ...
 const record={...summary,round:next.round},index=memory.scenes.findIndex(s=>s.place===summary.place);
 if(index<0)memory.scenes.push(record);else memory.scenes[index]=record;
 for(const outcome of proposal.outcomes)if(outcome.kind!=='waiting')memory.spotlight[outcome.playerId]=next.round;
 // Scope expiry is deterministic. A stale follow instruction cannot carry its
 // owner out of the agreed area on a later turn.
 for(const e of memory.entries)if(e.kind==='agreement'&&e.status==='active'&&!e.places.includes(next.location))e.status='expired';
 next.memory=memory;
}
