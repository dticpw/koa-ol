...
async function bodyOf(request){
 ...
 const fields={hello:['name'],join:['table'],leave:['table'],start:['table','requestId'],chat:['table','text','requestId'],act:['table','text','hold','round'],withdraw:['table','round'],resolve:['table','requestId','round'],close:['table']}[body.op];
...
async function view(db,row,player){
...
export function createMultiplayerHandler({resolver=resolveMultiplayer}={}){
 return async({request,env})=>{
  let release=null;const db=env.DB;
  try{
   ...
   if(player){await run(db,'UPDATE koa_fiction_multi_players SET last_seen=?,expires_at=? WHERE token_hash=? AND last_seen<?',now,now+TTL,tokenHash,now-30000);player=await one(db,'SELECT * FROM koa_fiction_multi_players WHERE token_hash=?',tokenHash);}
   if(!body){
    ...
    const base={you:player?{...publicPlayer(player),table:player.table_no}:null,story:{id:story.id,title:story.title,minPlayers:story.minPlayers,maxPlayers:story.maxPlayers}};
    if(n!==null)return json({...base,table:await view(db,await readRoom(db,n),player)});
    const rows=await all(db,'SELECT * FROM koa_fiction_multi_tables ORDER BY table_no');
    const tables=[];for(const row of rows){const v=await view(db,row,player);delete v.game;tables.push(v);}return json({...base,tables});
   }
   ...
   const commit=async(next,status=row.status,requestId=null)=>{
    const commitTime=Date.now(),statements=[];
    ...
    const results=await db.batch(statements);if(!changed(results.at(-1)))error(409,'本次操作的锁已过期，请刷新查看进度。','revision_conflict');release=null;
   };
   if(body.op==='join'){
    ...
    if(row.status!=='waiting')error(409,'这桌已经开局，请选择空桌。','already_started');
    if(seats.length>=story.maxPlayers)error(409,'这桌已经满员。','table_full');
    const joined=await run(db,'UPDATE koa_fiction_multi_players SET table_no=?,join_order=?,last_seen=? WHERE token_hash=? AND table_no IS NULL',body.table,(seats.at(-1)?.join_order||0)+1,now,tokenHash);
    if(!changed(joined))error(409,'你已在另一桌，请刷新。','already_seated');
    await commit(null,'waiting');return json({ok:true,table:body.table});
   }
   if(!seats.some(p=>p.player_id===player.player_id)){
    if(body.op==='leave')return json({ok:true});error(403,'只有本桌成员可以操作。','not_member');
   }
   if(['start','resolve','close'].includes(body.op)&&hostId!==player.player_id)error(403,'只有房主可以执行这项操作。','host_required');
   if(body.op==='leave'||body.op==='close'){
    const emptied=body.op==='close'||seats.length===1;
    if(emptied&&state)await run(db,'INSERT OR REPLACE INTO koa_fiction_multi_archives(run_id,table_no,state_json,archived_at) VALUES (?,?,?,?)',state.runId,body.table,JSON.stringify(state),now);
    await run(db,'UPDATE koa_fiction_multi_players SET table_no=NULL,join_order=NULL WHERE '+(body.op==='close'?'table_no=?':'player_id=?'),body.op==='close'?body.table:player.player_id);
    if(state){delete state.drafts[player.player_id];state.activePlayers=seats.filter(p=>p.player_id!==player.player_id).map(p=>p.player_id);}
    await commit(emptied?null:state,emptied?'waiting':row.status);return json({ok:true});
   }
   if(body.op==='start'){
   ...
   if(body.op==='resolve'){
    ...
    const result=await resolver({env,db,state,actions,chat});await commit(result.state,result.state.ended?'ended':'playing',body.requestId);return json({ok:true,round:result.state.round});
   }
