...
import {assertParticipants} from './scenes.js';
...
export const proposalSchema=object({destination:str,route:list(str,4),suspicionDelta:{type:'integer',minimum:-1,maximum:2},suspicionReason:str,progress:object({}),ended:bool,
...
export function createAdventure(members,story){
 ...
 return {version:3,runId:crypto.randomUUID(),hostRevision:'cooperative-v3',storyId:story.id,storyRevision:story.revision,round:0,location:story.start,sceneId:'main',
...
export function applyRuling(state,p,actions){
 ...
 const event={round:next.round,sceneId:state.sceneId,audience:[...scene.participants],from:state.location,location:next.location,route:[...p.route],outcomes:structuredClone(p.outcomes),suspicionReason:p.suspicionReason,memoryIds:p.memoryUpdates.map(m=>m.id),coordination};
...
export async function resolveMultiplayer({env,db,state,actions,chat=[],fetchImpl=fetch}){
 ...
 try{for(let attempt=0;attempt<2;attempt++){
  const raw=await invoke([{role:'developer',content:RULES},{role:'user',content:JSON.stringify({...context,...(issue?{correction:issue}:{})})}],{type:'json_schema',name:'cooperative_ruling_v3',strict:true,schema:schemas.proposal},3400);
