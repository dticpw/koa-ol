...
export function applyMemory(state,next,proposal,actions,story){
 ...
 for(const raw of proposal.memoryUpdates){
  ...
  if(Array.isArray(update.evidence)){
   ...
   sources=update.evidence.map(ref=>resolveEvidence(state,actions,proposal,ref));
   // An event needs a settled outcome as its primary source; an authenticated
   // action may accompany it as context, never establish the event by itself.
   if(update.basis==='event')sources=[...sources.filter(s=>s.actor==='host'),...sources.filter(s=>s.actor!=='host')];
   update.source=sources[0];delete update.evidence;
