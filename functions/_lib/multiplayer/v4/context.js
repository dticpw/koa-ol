import {projectScene} from '../v3/scenes.js';
...
export function selectedPlaces(state,actions,story){
 ...
 // Include full requested routes; scene changes alone do not end a turn.
 // The complete graph also carries constraints for alternate/colloquial routes.
 ...
 while(queue.length){const route=queue.shift(),last=route.at(-1);if(targets.includes(last))for(const id of route)selected.add(id);for(const id of story.places[last][1])if(!seen.has(id)){seen.add(id);queue.push([...route,id]);}}
 return [...selected];
}
...
export function buildContext(state,actions,chat=[],story){
 ...
 return {
  ...
  sceneSummaries:state.memory.scenes.filter(s=>places.includes(s.place)),recentEvents:state.recentEvents.map(({round,from,location,route,outcomes,suspicionReason,turnEnd})=>({round,from,location,route,outcomes,suspicionReason,turnEnd})),
