...
const narrationFormat={type:'json_schema',name:'reviewed_narration',strict:true,schema:object({consistent:{type:'boolean'},issue:str,narration:str})};
...
const NARRATOR=`你是本轮的忠实叙述者兼一致性复核者。读取原话、行动前世界、已结算步骤与行动后世界，返回JSON。先检查：是否保留玩家意图与明确限制；关键行为有没有被偷换（点火不能改成看）；必要的合理准备和短连贯步骤是否被无故拒绝；来源/材质是否凭空改变；叙述结果是否符合实体位置、损耗、门与悬锤、时间；未知结果是否被宣称全面安全。如果存在实质矛盾，consistent=false并在issue指出，narration为空。无需重新判定每一种常识物理细节：主持被授权裁定一般局部用途，未预写不是矛盾，有依据的失败也可以通过。不要把轻微文学修辞或不同合理裁量当作失败。
通过时consistent=true，issue为空，正常操作叙述约360—440个汉字、2—4自然段；目标是中文字符数而不是token数，正常操作不要缩成一两百字，先回应玩家做了什么，再交代结果和现在的处境。只从已结算事实展开，保留部分完成和停止点；不擅自执行未提交动作，不复活消耗品，不新增暗门/奖品/NPC。物品事实是当前快照，不是下一步建议。火焰观察只能说明落点有限情况，不能证明空气安全或所有机关已排除。若本轮只有澄清或简短客观失败，可少于300字，直接自然地说明。全局状态不等于玩家的视野，不要确认玩家此刻未观察的远处陈设。只在玩家本轮探查空气或机关时解释有限观察的边界，不必每轮重复安全说明。不要重复一大段旧环境，不列选项，不提JSON、验证器、规则引擎。若时间用尽就收束，不编造新的一天。`;
export async function resolveLabTurn({state,body,call}){
 ...
 const before=hostContext(state);
 let proposal,applied;
 try{
  const raw=await call([{role:'developer',content:HOST},{role:'user',content:JSON.stringify({world:before,player_action:action})}],{format:proposalFormat,maxTokens:3600,timeoutMs:45000});
  try{proposal=JSON.parse(raw);applied=applyProposal(state,proposal,action);}catch{throw new ApiError(503,'主持本轮的行动记录没有通过一致性检查，进度未改变。请重试这次尝试。','adjudication_failed');}
  const rawNarrative=await call([{role:'developer',content:NARRATOR},{role:'user',content:JSON.stringify({player_action:action,before,confirmed_steps:applied.outcomes,after:hostContext(applied.state),ending:applied.state.ending})}],{format:narrationFormat,maxTokens:2200,timeoutMs:45000});
  let result;try{result=JSON.parse(rawNarrative);}catch{throw new ApiError(503,'主持的回应没有完整返回，进度未改变，请重试。','adjudication_failed');}
  if(result.consistent!==true||typeof result.narration!=='string'||!result.narration.trim())throw new ApiError(503,'主持对本轮后果的描述出现矛盾，进度未改变，请重试。','adjudication_failed');
