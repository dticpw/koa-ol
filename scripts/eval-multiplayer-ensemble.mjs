...
let state,memory='';try{state=JSON.parse(await fs.readFile(path.join(out,'state.json'),'utf8'));}catch{state=createAdventure(members,revision);state.hostModel=args['--host-model']||'gpt-5.6-sol';}
if((state.hostModel||'gpt-5.6-sol')!==(args['--host-model']||'gpt-5.6-sol'))throw Error('Cannot change a running evaluation model');
...
try{while(state.round<maxRounds&&!state.ended){
 ...
 }}catch(e){console.error(e.code||e.message);process.exitCode=1;}finally{const files=await fs.readdir(out);await fs.writeFile(path.join(out,'summary.json'),JSON.stringify({revision,hostModel:state.hostModel||'gpt-5.6-sol',round:state.round,ended:state.ended,location:state.location,keyCopied:state.progress.keyCopied,totalFailureFiles:files.filter(f=>/^(driver-)?failure-/.test(f)).length,spentMicro:(await db.prepare('SELECT SUM(spent_micro) n FROM koa_fiction_budget').first()).n,scope:'Three model-simulated players. Experience cues predeclared, no GM secrets in player driver.'},null,2));sql.close();}
