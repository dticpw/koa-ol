...
while(state.round<maxRounds&&!state.ended){
 ...
 const start=Date.now();try{
  ...
  console.log(JSON.stringify({suite,revision,round:state.round,location:state.location,keyCopied:state.keyCopied,ended:state.ended,seconds:Math.round((Date.now()-start)/1000),memory:state.memory?.entries.length}));errors=0;
...
await fs.writeFile(path.join(out,'summary.json'),JSON.stringify({revision,suite,round:state.round,ended:state.ended,keyCopied:state.keyCopied,location:state.location,errors,spentMicro:(await db.prepare('SELECT SUM(spent_micro) AS n FROM koa_fiction_budget').first()).n},null,2));sql.close();
