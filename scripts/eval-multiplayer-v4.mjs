...
const report={revision,kind:'synthetic-and-labeled-real-checkpoint-probes',cases:[]};
try{for(let repeat=1;repeat<=repeats;repeat++)for(const c of cases.filter(c=>!args['--only']||args['--only'].split(',').includes(c.id))){
 ...
 const state=await c.make(),actions=c.actions.map((text,i)=>({playerId:state.characters[i].id,name:state.characters[i].name,text,hold:false}));await fs.writeFile(path.join(out,stem+'-input.json'),JSON.stringify({source:c.source||'synthetic',state,actions},null,2));let result;
