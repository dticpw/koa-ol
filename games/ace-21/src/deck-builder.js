...
export function validateDeck(entries, catalog, rules) {
  ...
  for (const e of entries) {
    if (!e || typeof e.key !== 'string' || !Number.isInteger(e.count) || e.count<1 || e.count>rules.cardCount) {
      errors.push('卡牌数量必须为1至40的整数'); continue;
    }
...
export function parseDeck(input, catalog, rules) {
  ...
  const entries=data.entries.map(e=>{
    if (!e || typeof e.key!=='string' || !index.get(e.key)?.deckEligible || seen.has(e.key)
      || !Number.isInteger(e.count) || e.count<1 || e.count>rules.cardCount) throw new Error('牌组含无效卡牌或数量，请检查文件');
...
export function loadCollection(raw, catalog, rules) {
  ...
  const decks=data.decks.map(d=>{
    ...
    ids.add(d.id);return {id:d.id,...parseDeck({...d,version:1},catalog,rules)};
  });
