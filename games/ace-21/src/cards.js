import { buildCatalog, validateDeck, parseDeck, serializeDeck, loadCollection, STORAGE_KEY } from './deck-builder.js?v=102';
import { toneLabel } from './feedback.js?v=102';
...
function changeCard(key,delta){
  ...
  if(delta>0&&result().count>=rules.cardCount){notify('牌组已满24张，请先移除卡牌');return;}
...
function updateDetailButton(){const c=index.get(selected),full=result().count>=rules.cardCount;const button=$('detail-add');button.disabled=!c.deckEligible||full;button.textContent=!c.deckEligible?(c.status==='special'?'希儿专属 · 不参与组牌':'待实装 · 暂不能组牌'):full?'牌组已满24张':`加入牌组${quantity(c.key)?` · 已选${quantity(c.key)}张`:''}`;}
function showDetail(key){
  const c=index.get(key);if(!c)return;selected=key;
  $('detail-content').innerHTML=`<img class="detail-art" src="../assets/trump-${c.art}.webp" alt=""><p class="detail-meta">${toneLabel[c.tone]} · ${c.power===null?'希儿专属':`${c.power} 牌力`}${c.status==='planned'?' · 待实装':''}</p><h2 id="detail-title">${esc(c.name)}</h2><p class="detail-effect">${esc(c.text)}</p><p class="detail-note">${c.status==='ordinary'?`${c.stay?'持续牌，留在桌面时生效。':'瞬时牌，结算效果后不留在桌面。'}${c.powerStatus==='working-proposal'?'牌力为初始暂定值。':''}`:c.status==='special'?'仅用于希儿对战，不可加入玩家的24张牌组。':'已采纳设计，尚未接入规则引擎；完整交互与结算细节待定稿。'}</p>`;
...
async function init(){
  try{
    const responses=await Promise.all([fetch('../data/card-balance.json?v=102'),fetch('../data/planned-cards.json?v=102')]);
