import { buildCatalog, validateDeck, parseDeck, serializeDeck, loadCollection, STORAGE_KEY } from './deck-builder.js?v=104';
import { toneLabel } from './feedback.js?v=104';
...
async function init(){
  try{
    const responses=await Promise.all([fetch('../data/card-balance.json?v=104'),fetch('../data/planned-cards.json?v=104')]);
